...
public:
	HexagonAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 100; }
	void Draw() const override {
...
enum class WeaponType { LASER, BULLET, SPREAD, BOMB, COUNT };
class Projectile {
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			Rectangle lr = { transform.position.x - 2.f, transform.position.y - 30.f, 4.f, 30.f };
			DrawRectangleRec(lr, RED);
		}
		else if (type == WeaponType::BOMB) {
			DrawCircleV(transform.position, 16.f, ORANGE);
		}
		else if (type == WeaponType::SPREAD) {
			DrawCircleV(transform.position, 6.f, SKYBLUE);
		}
	}
	...
	float GetRadius() const {
		switch (type) {
		case WeaponType::BULLET: return 5.f;
		case WeaponType::LASER: return 2.f;
		case WeaponType::BOMB: return 16.f;
		default: return 5.f;
		}
	}
...
inline static Projectile MakeProjectile(WeaponType wt, const Vector2 pos, float speed, float angleDeg = 0.0f)
{
    float angleRad = DEG2RAD * angleDeg;
    Vector2 dir = { 0.f, -1.f };
    dir = Vector2Rotate(dir, angleRad);
    Vector2 vel = Vector2Scale(dir, speed);

    int dmg = 10;
    if (wt == WeaponType::LASER) dmg = 20;
    else if (wt == WeaponType::SPREAD) dmg = 15;
    else if (wt == WeaponType::BOMB) dmg = 100;
...
public:
	...
	float GetFireRate(WeaponType wt) const
	{
		...
		case WeaponType::SPREAD: return fireRateBullet;
		case WeaponType::BOMB:   return fireRateBomb;
		default:                 return fireRateBullet;
		}
	}
...
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		texture = LoadTexture("papiez.png");
				...
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						...
						if (currentWeapon == WeaponType::SPREAD) {
							projectiles.push_back(MakeProjectile(currentWeapon, p, projSpeed, -15.f));
							projectiles.push_back(MakeProjectile(currentWeapon, p, projSpeed, 0.f));
							projectiles.push_back(MakeProjectile(currentWeapon, p, projSpeed, 15.f));
						}
						else {
